...
class TestMainPage:
    ...
    def test_verify_feed_transfer(self, browser_driver):
        main_page = MainPage(browser_driver)
        main_page.redirect_to_feed(MainPageLocators.feed_button)
        result = browser_driver.current_url
        assert result == Urls.feed_url

    @allure.title('Переход по клику на "Конструктор"')
    def test_transfer_to_constructor(self, browser_driver):
        main_page = MainPage(browser_driver)
        main_page.redirect_to_constructor(MainPageLocators.feed_button, MainPageLocators.constructor_button)
        result = browser_driver.current_url
        assert result == Urls.main_url

    @allure.title('Проверка открытия поп-апа с информацией об ингредиенте')
    def test_new_window_opening(self, browser_driver):
        main_page = MainPage(browser_driver)
        result = main_page.open_new_window(MainPageLocators.object_order, MainPageLocators.ingredient_details)
    ...
    def test_verify_ingredient_added(self, browser_driver):
        main_page = MainPage(browser_driver)
